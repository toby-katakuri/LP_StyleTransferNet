...
from edgeenhance_modules import Trans_mid_encoder_image, Trans_high, vgg4, NLayerDiscriminator
...
class EdgeEnhanceNet(nn.Module):
    def __init__(self, vgg_path='vgg_normalised.pth'):
        super(EdgeEnhanceNet, self).__init__()


        self.net_mid_encoder_image = Trans_mid_encoder_image().cuda()
        ...
        self.optimizer = torch.optim.Adam([{'params': self.net_mid_encoder_image.parameters()}, {'params': self.net_high.parameters()}], lr=1e-4)
    ...
    def set_input(self, c_pyr, c, s, sed, c_edge):
        ...
        self.c_edge = c_edge.cuda()

    def forward_net(self):
        sedr_mid = self.net_mid_encoder_image(self.c_low, self.sed_low, self.cr_mid, self.c_edge)

        sed_mid = F.interpolate(self.sed_low, [256, 256]) + sedr_mid
        #sed_mid = F.interpolate(self.c_low, [256, 256]) + sedr_mid
    ...
    def backward_G(self):
        ...
        # calu
        self.loss_G_1 = 1 * self.loss_c + 80 * self.loss_style_remd2 + 50 * self.loss_content_relt + 50 * self.loss_s
        self.loss_G = self.loss_G_1 + 1 * self.loss_gan_g
    ...
    def backward_D(self):
        ...
        self.loss_D = (self.loss_gan_d_fake + self.loss_gan_d_real) / 2
    ...
    def save_para(self, path):
        torch.save(self.net_mid_encoder_image.state_dict(), path + '/net_mid_encoder.pt')
...
class EdgeEnhanceNet_Test(nn.Module):
    def __init__(self, path):
        super(EdgeEnhanceNet_Test, self).__init__()
        ...
        self.net_mid_encoder_image.load_state_dict(torch.load(path + r'\net_mid_encoder.pt'))
    ...
    def test_1image(self, batch_id):
        self.forward_net()
        torchvision.utils.save_image(self.c, '{}_c.jpg'.format(batch_id))
        torchvision.utils.save_image(self.sed_high, '{}_se.jpg'.format(batch_id))
